...
class eleAnalysis(LeptonAnalysis):
    def __init__(self):
        LeptonAnalysis.__init__(self)
...
class muAnalysis(LeptonAnalysis):
    def __init__(self):
        LeptonAnalysis.__init__(self)
    ...
    def analyze(self, event):
        ...
        for mu_idx, mu in enumerate(muons):
            ...
            isTruthMatch = (mu.genPartIdx >= 0 and mu.genPartFlav==1) #FIXME
            if isTruthMatch:
...
p = PostProcessor(".", files, cut=preselection, branchsel=None, modules=[
                  muAnalysis()], noOut=True, histFileName=o.output, histDirName="plots")
